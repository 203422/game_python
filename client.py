...
player_state = {'x': 250, 'y': 570, 'ready': False, 'shoot': False, 'lives': 2}
state_movements = {}
lasers = []
hamsters = []
lasers_hamsters = []
...
def handle_collisions(player_hit):
    if player_hit:
        print('¡Has sido impactado por un láser del hamster!')


async def update_state(socket):
    global state_movements, lasers, hamsters, lasers_hamsters, players_ready, player_count
    ...
    if data:
        ...
        handle_collisions(state.get('player_hit', None))
...
async def main():
    ...
    async with websockets.connect(f"ws://{host}:{port}") as socket:
        ...
        while running:
            ...
            for hamster in hamsters:  # Dibujar los hamsters con sus nuevas posiciones
